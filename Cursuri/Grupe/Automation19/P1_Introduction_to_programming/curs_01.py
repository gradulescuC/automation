...
#
# var_float=1.245
# print(f"noua valoare este {int(var_float)}")
...
# variabila = input("Va rugam introduceti varsta: ")
...
sentence = "My name is python and i am a snake"
print(sentence[0])
print(sentence[:])
...
print(sentence[-5:]) # prints the characters starting from the fifth character from the end
print(sentence[:6]) # My nam
print(sentence[:8]) # My name. The end of the interval is not taken into account
print(sentence[::3])
...
# b) string methods
# print(len(sentence))  # the len function is useful to check how many characters are in the string
# print(sentence.count('a')) # counts how many times do we have the character a in the string
# print(sentence.upper()) # upper is used to convert all the characters into capitals
# print(sentence.replace('i','y')) # replace is used to replace a character with another character
...
print(f"The last letter in \"{myStr}\" is \"{myStr[last_index]}\"") # in order to print the double quotes on the screen we need to use the escape character (\)
print(myStr[0:last_index]) # this sintaxt will not print the last character in the string
print(myStr[0:])
print(myStr[:])
