...
class Browser():
		# Am instantiat un obiect din clasa chrome
		chrome = webdriver.Chrome(executable_path=ChromeDriverManager().install())
		# prin intermediul obiectului am accesat metoda maximize_window pentru a mari fereastra la dimensiune maxima
		chrome.maximize_window()

		# Am setat un timp de asteptare implicit de doua secunde
		...
		def close(self):
						# Am apelat metoda quit din clasa Chrome prin intermediul obiectului instantiat
						# quit = metoda care inchide toata instanta de browser
						# close = metoda care inchide doar tab-ul activ din broswer
						self.chrome.quit()